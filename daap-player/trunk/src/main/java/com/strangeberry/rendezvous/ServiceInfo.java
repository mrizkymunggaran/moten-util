...
public class ServiceInfo extends Rendezvous.Listener {
     ...
     * @param addr
     *            the address to which the service is bound
     * @param port
     *            the local port on which the service runs
     * @param text
     *            string describing the service
     */
    public ServiceInfo(String type, String name, InetAddress addr, int port,
            String text) {
    ...
    /**
     * Construct a service description for registrating with Rendezvous. The
    ...
    public ServiceInfo(String type, String name, InetAddress addr, int port,
        ...
        try {
            ...
            for (Enumeration e = props.keys(); e.hasMoreElements();) {
                ...
                } else if (val != NO_VALUE) { throw new IllegalArgumentException(
    ...
    ServiceInfo(String type, String name) {
        if (!type.endsWith(".")) { throw new IllegalArgumentException(
                "type must be fully qualified DNS name ending in '.': " + type); }
        if (name.endsWith(".")) {
            if (!name.endsWith("." + type)) { throw new IllegalArgumentException(
                    "service name has the wrong type: name=" + name + ", type="
                            + type); }
        } else {
            name = name + "." + type;
        }
    ...
    public String getName() {
        if ((type != null) && name.endsWith("." + type)) { return name
                .substring(0, name.length() - (type.length() + 1)); }
        return name;
    }
     ...
     * protocol, addres, port, and path properties.
     */
        ...
        if ((rec != null) && !rec.isExpired(now)) {
            ...
            case TYPE_SRV:
                if (rec.name.equals(name)) {
                    ...
                    addr = null;
                    updateRecord(rendezvous, now, (DNSRecord) rendezvous.cache
                            .get(server, TYPE_A, CLASS_IN));
                }
                break;
            case TYPE_TXT:
                if (rec.name.equals(name)) {
    ...
    /**
     * Update the server information from the cache, send out repeated DNS
     * queries for updated information.
     */
    boolean request(Rendezvous rendezvous, long timeout) {
        ...
        try {
            rendezvous.addListener(this, new DNSQuestion(name, TYPE_ANY,
                    CLASS_IN));
            while (server == null || addr == null || text == null) {
                ...
                if (next <= now) {
                    ...
                    if (server != null) {
                        out.addQuestion(new DNSQuestion(server, TYPE_A,
                                CLASS_IN));
                    }
                    out.addAnswer((DNSRecord) rendezvous.cache.get(name,
                            TYPE_SRV, CLASS_IN), now);
                    out.addAnswer((DNSRecord) rendezvous.cache.get(name,
                            TYPE_TXT, CLASS_IN), now);
                    if (server != null) {
                        out.addAnswer((DNSRecord) rendezvous.cache.get(server,
                                TYPE_A, CLASS_IN), now);
                    }
                    rendezvous.send(out);
    ...
    public boolean equals(Object obj) {
        return (obj instanceof ServiceInfo)
                && name.equals(((ServiceInfo) obj).name);
    }
