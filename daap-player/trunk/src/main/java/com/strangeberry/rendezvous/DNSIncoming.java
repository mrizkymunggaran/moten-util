...
final class DNSIncoming extends DNSConstants {
    ...
    int get(int off) throws IOException {
        if ((off < 0) || (off >= len)) { throw new IOException(
                "parser error: offset=" + off); }
    ...
    String readName() throws IOException {
        ...
        while (true) {
            ...
            case 0xC0:
                ...
                if (off >= first) { throw new IOException(
            ...
            for (int off = 0, len = packet.getLength(); off < len; off += 32) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    System.out.print(Integer
                            .toHexString((data[off + i] & 0xF0) >> 4));
                    System.out.print(Integer
                            .toHexString((data[off + i] & 0x0F) >> 0));
                }
                System.out.println();
                System.out.print("    ");
                for (int i = 0; i < n; i++) {
