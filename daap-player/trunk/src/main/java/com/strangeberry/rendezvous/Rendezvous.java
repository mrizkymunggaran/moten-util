...
public class Rendezvous extends DNSConstants {
    ...
    static int debug = Integer.parseInt(System.getProperty("rendezvous.debug",
            "0"));
    InetAddress group;
    ...
    /**
     * Create an instance of Rendezvous and bind it to a specific network
    ...
    /**
     * Return the address of the interface to which this instance of Rendezvous
     * is bound.
     */
     ...
     * @param type
     *            full qualified service name, such as
     ...
     */
    public synchronized void addServiceListener(String type,
    ...
    /**
     * Register a service. The service is registered for access by other
     * rendezvous clients. The name of the service may be changed to make it
     * unique.
     */
    public void registerService(ServiceInfo info) throws IOException {
        try {
            ...
            for (int i = 0; i < 3;) {
                ...
                out.addAnswer(new DNSRecord.Pointer(info.type, TYPE_PTR,
                        CLASS_IN, DNS_TTL, info.name), 0);
                out.addAnswer(new DNSRecord.Service(info.name, TYPE_SRV,
                        CLASS_IN, DNS_TTL, info.priority, info.weight,
                        info.port, info.name), 0);
                out.addAnswer(new DNSRecord.Text(info.name, TYPE_TXT, CLASS_IN,
                        DNS_TTL, info.text), 0);
                out.addAnswer(new DNSRecord.Address(info.name, TYPE_A,
                        CLASS_IN, DNS_TTL, info.getIPAddress()), 0);
                send(out);
                i++;
                nextTime += 225;
            }
    ...
    public void unregisterService(ServiceInfo info) {
        try {
            ...
            for (int i = 0; i < 3;) {
                ...
                DNSOutgoing out = new DNSOutgoing(FLAGS_QR_RESPONSE | FLAGS_AA);
                out.addAnswer(new DNSRecord.Pointer(info.type, TYPE_PTR,
                        CLASS_IN, 0, info.name), 0);
                out.addAnswer(new DNSRecord.Service(info.name, TYPE_SRV,
                        CLASS_IN, 0, info.priority, info.weight, info.port,
                        info.name), 0);
                out.addAnswer(new DNSRecord.Text(info.name, TYPE_TXT, CLASS_IN,
                        0, info.text), 0);
                out.addAnswer(new DNSRecord.Address(info.name, TYPE_A,
                        CLASS_IN, 0, info.getIPAddress()), 0);
                send(out);
                i++;
                nextTime += 125;
            }
            ...
            for (int i = 0; i < 3;) {
                ...
                for (Enumeration e = services.elements(); e.hasMoreElements();) {
                    ...
                    out.addAnswer(new DNSRecord.Service(info.name, TYPE_SRV,
                            CLASS_IN, 0, info.priority, info.weight, info.port,
                            info.name), 0);
                    out.addAnswer(new DNSRecord.Text(info.name, TYPE_TXT,
                            CLASS_IN, 0, info.text), 0);
                    out.addAnswer(new DNSRecord.Address(info.name, TYPE_A,
                            CLASS_IN, 0, info.getIPAddress()), 0);
                }
                send(out);
     ...
     */
    void checkService(ServiceInfo info) throws IOException,
        ...
        for (int i = 0; i < 3;) {
            for (Iterator j = cache.find(info.type); j.hasNext();) {
                ...
                if ((a.type == TYPE_PTR) && !a.isExpired(now)
                        && info.name.equals(((DNSRecord.Pointer) a).alias)) {
            ...
            out.addAuthorativeAnswer(new DNSRecord.Pointer(info.type, TYPE_PTR,
                    CLASS_IN, DNS_TTL, info.name));
            send(out);
            i++;
            nextTime += 175;
        }
    ...
    static abstract class Listener extends DNSConstants {
         ...
         * Update a DNS record.
         */
        abstract void updateRecord(Rendezvous rendezvous, long now,
                DNSRecord record);
    }

    /**
     * Add a listener for a question. The listener will receive updates to of
     ...
     * registered records.
     */
    synchronized void handleQuery(DNSIncoming in, InetAddress addr, int port)
            throws IOException {
            ...
            case TYPE_PTR:
                ...
                for (Enumeration s = services.elements(); s.hasMoreElements();) {
                    ...
                    if (q.name.equals(info.type)) {
                        ...
                        out.addAnswer(in, new DNSRecord.Pointer(info.type,
                                TYPE_PTR, CLASS_IN, DNS_TTL, info.name));
                    }
                }
                break;
            default:
                ...
                if (info != null) {
                    ...
                    if ((q.type == TYPE_SRV) || (q.type == TYPE_ANY)) {
                        out.addAnswer(in, new DNSRecord.Service(q.name,
                                TYPE_SRV, CLASS_IN | CLASS_UNIQUE, DNS_TTL,
                                info.priority, info.weight, info.port,
                                info.name));
                    }
                    if ((q.type == TYPE_TXT) || (q.type == TYPE_ANY)) {
                        out.addAnswer(in, new DNSRecord.Text(q.name, TYPE_TXT,
                                CLASS_IN | CLASS_UNIQUE, DNS_TTL, info.text));
                    }
                    if ((q.type == TYPE_A) || (q.type == TYPE_ANY)) {
                        out.addAnswer(in, new DNSRecord.Address(q.name, TYPE_A,
                                CLASS_IN | CLASS_UNIQUE, DNS_TTL, info
                                        .getIPAddress()));
                    }
                }
            }
                ...
                while (!done) {
                    ...
                    try {
                        ...
                        if (linklocal != isLinkLocal(from)) {
                            // Ignore linklocal packets on regular interfaces,
                            // unless this is
                            // also a linklocal interface. This is to avoid
                            // duplicates. This is
                            // a terrible hack caused by the lack of an API to
                            // get the address
                            // of the interface on which the packet was
                            // received.
                            continue;
                        }
                        if (loopback != isLoopback(from)) {
                            // Ignore loopback packets on a regular interface
                            // unless this is
                        ...
                        if (msg.isQuery()) {
                            if (packet.getPort() != MDNS_PORT) {
                                handleQuery(msg, packet.getAddress(), packet
                                        .getPort());
                            }
            ...
            if ((rec.type == TYPE_PTR) && rec.name.equals(type)) {
                ...
                if ((old == null) && !expired) {
                    ...
                    list.addLast(new Event(name) {
                        ...
                        void send() {
                            listener.addService(Rendezvous.this, type,
                                    this.name);
                        }
                    });
                    ...
                    list.addLast(new Event(name) {
                        ...
                        void send() {
                            listener.removeService(Rendezvous.this, type,
                                    this.name);
                        }
                    });
                    return;
                ...
                while (true) {
                    ...
                    synchronized (Rendezvous.this) {
                        ...
                        if (nextTime <= now) {
                            ...
                            out.addQuestion(new DNSQuestion(type, TYPE_PTR,
                                    CLASS_IN));
                            for (Enumeration e = services.elements(); e
                                    .hasMoreElements();) {
                                DNSRecord rec = (DNSRecord) e.nextElement();
        ...
        public void run() {
            ...
            for (Enumeration e = browsers.elements(); e.hasMoreElements();) {
                ...
                if (browser.type.equalsIgnoreCase(info.type)) {
                    browser.listener.resolveService(Rendezvous.this, info.type,
                            info.name, result);
                }
            }
        }
