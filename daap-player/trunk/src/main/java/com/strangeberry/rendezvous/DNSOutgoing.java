...
final class DNSOutgoing extends DNSConstants {
    ...
    DNSOutgoing(int flags, boolean multicast) {
        ...
        names = new Hashtable();
        if (multicast)
        	data = new byte[MAX_MSG_ABSOLUTE]; 	// Changed to ABS when getting buffer full exceptions
        else									// TYPICAL*3 seemed to work, but wanted some extra margin 
    ...
    void writeByte(int value) throws IOException {
        if (off >= data.length) { throw new IOException("buffer full " + flags); }
        data[off++] = (byte) value;
    }
