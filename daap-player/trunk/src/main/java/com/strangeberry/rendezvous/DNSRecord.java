...
// @(#)DNSRecord.java, 1.24, 01/14/2003
//
...
abstract class DNSRecord extends DNSEntry {
    ...
    boolean suppressedBy(DNSIncoming msg) {
        for (int i = msg.numAnswers; i-- > 0;) {
            if (suppressedBy((DNSRecord) msg.answers.elementAt(i))) { return true; }
        }
        return false;
    }
    ...
    boolean suppressedBy(DNSRecord other) {
        if (sameAs(other) && (other.ttl > ttl / 2)) { return true; }
        return false;
    }
    ...
    static class Address extends DNSRecord {
        ...
        public String toString() {
            return toString(getHostAddress());
        }
    }
    ...
    static class Text extends DNSRecord {
        ...
        boolean sameValue(DNSRecord other) {
            Text txt = (Text) other;
            if (txt.text.length != text.length) { return false; }
            for (int i = text.length; i-- > 0;) {
                if (txt.text[i] != text[i]) { return false; }
            }
            return true;
        }
    ...
    static class Service extends DNSRecord {
        ...
        Service(String name, int type, int clazz, int ttl, int priority,
