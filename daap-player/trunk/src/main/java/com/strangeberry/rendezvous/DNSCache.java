...
/**
 * A table of DNS entries. This is a closed hash table which can handle multiple
 * entries with the same name. It provides iterators to efficiently iterate over
 * all the entries with a given name.
 * 
...
class DNSCache {
    ...
    /**
     * Add an entry to the table. The table will be grown if it is more than 75%
     * full.
     */
    ...
    DNSEntry get(String name, int type, int clazz) {
        ...
        while (true) {
            DNSEntry e = entries[i];
            if (e == null) { return null; }
            if (key.equals(e.key) && (type == e.type) && (clazz == e.clazz)) { return e; }
            if (--i < 0) {
                i = entries.length - 1;
            }
        }
    ...
    private class IterateAll implements Iterator {
        ...
        public boolean hasNext() {
            for (; index >= 0; index--) {
                if (entries[index] != null) { return true; }
            }
            return false;
        }
