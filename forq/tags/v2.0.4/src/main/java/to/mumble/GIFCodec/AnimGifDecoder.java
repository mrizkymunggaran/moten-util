...
//		dbg(v);
	...
	public boolean canAccept() throws IOException
	{
		...
		{
			m_is.mark(10);
			if(m_is.read(ar) != 6) return false;		// Cannot read -> invalid,
		...
		{
			try {	m_is.reset();	} catch(Exception x) {}
		}
	}
...
//		dbg("End of GLOBAL data");
	...
	private void createHandler(int bufimatype) throws IOException
	{
		switch(bufimatype)
		{
			...
			case BufferedImage.TYPE_BYTE_INDEXED:	m_h	= new GifIndexedHandler(this, m_im, bufimatype);	break;
		}

		m_bi	= m_h.prepare();					// Prepare the read,
		if(m_bi == null) throw new IOException("Unexpected format of recognised BufferedImage type!?");
	}
...
//					dbg("Graphics Control Extension found");
	...
	/**
	 *	Called by the decompressor when it has found a set of pixels. This method
	 ...
	 *
	 *
	 *	Runs the data decompressor, calling the image writer for each set of
...
//				int	oc	= oldcode;
	...
	{
		if(m_bitpos + m_codesize > 32) return m_eofcode;// No more data!
