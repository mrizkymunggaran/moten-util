...
import moten.david.util.math.gui.VennDiagramEdwardsPanel;
...
public class VennPanel extends javax.swing.JPanel {
	...
	public VennPanel(final VennInfo vennInfo) {
		...
		maxStandardErrors = Preferences.getInstance().getFloat(
				Preferences.VENN_MAX_STANDARD_ERRORS, maxStandardErrors);
		...
		add(confidence);
		confidence.setPreferredSize(new Dimension(100, confidence
				.getPreferredSize().height));

		threshold = new JSlider(0, 250);
		threshold.setValue(Math.round(1 / maxStandardErrors
				* threshold.getMaximum()));
		add(threshold);

		strategy = new JComboBox(FactorScoreStrategy.values());
		add(strategy);
		...
		int maxSelectable = 5;
		selector = new FactorSelector(vennInfo.getRotations().getLoadings()
				.columnCount(), maxSelectable);
		...
		vp = new VennDiagramEdwardsPanel(Math.min(maxSelectable, vennInfo
				.getRotations().getLoadings().columnCount()));
		add(vp);

		layout.putConstraint(SpringLayout.NORTH, confidenceLabel, 10,
				SpringLayout.SOUTH, strategy);
		layout.putConstraint(SpringLayout.WEST, confidenceLabel, 0,
				SpringLayout.WEST, confidence);
		layout.putConstraint(SpringLayout.EAST, confidenceLabel, 0,
				SpringLayout.EAST, confidence);

		layout.putConstraint(SpringLayout.NORTH, thresholdLabel, 0,
				SpringLayout.NORTH, confidenceLabel);
		layout.putConstraint(SpringLayout.WEST, thresholdLabel, 0,
				SpringLayout.WEST, threshold);
		layout.putConstraint(SpringLayout.EAST, thresholdLabel, 0,
				SpringLayout.EAST, threshold);

		layout.putConstraint(SpringLayout.NORTH, confidence, 0,
				SpringLayout.SOUTH, confidenceLabel);
		layout.putConstraint(SpringLayout.WEST, confidence, 10,
				SpringLayout.WEST, this);

		layout.putConstraint(SpringLayout.NORTH, threshold, 0,
				SpringLayout.NORTH, confidence);
		layout.putConstraint(SpringLayout.WEST, threshold, 10,
				SpringLayout.EAST, confidence);
		layout.putConstraint(SpringLayout.EAST, threshold, 0,
				SpringLayout.EAST, strategy);

		layout.putConstraint(SpringLayout.NORTH, strategy, 10,
				SpringLayout.NORTH, this);
		layout.putConstraint(SpringLayout.WEST, strategyLabel, 10,
				SpringLayout.WEST, this);
		layout.putConstraint(SpringLayout.VERTICAL_CENTER, strategyLabel, 0,
				SpringLayout.VERTICAL_CENTER, strategy);
		layout.putConstraint(SpringLayout.WEST, strategy, 3, SpringLayout.EAST,
				strategyLabel);

		layout.putConstraint(SpringLayout.NORTH, selectorScroll, 0,
				SpringLayout.NORTH, strategy);
		layout.putConstraint(SpringLayout.WEST, selectorScroll, 10,
				SpringLayout.EAST, strategy);
		layout.putConstraint(SpringLayout.SOUTH, selectorScroll, 0,
				SpringLayout.SOUTH, confidence);
		layout.putConstraint(SpringLayout.EAST, selectorScroll, 0,
				SpringLayout.EAST, this);
		layout.putConstraint(SpringLayout.NORTH, vp, 5, SpringLayout.SOUTH,
				confidence);
		layout.putConstraint(SpringLayout.SOUTH, vp, 0, SpringLayout.SOUTH,
				this);
	...
	private void updateVenn() {
		...
		FactorScoreStrategy strat = (FactorScoreStrategy) strategy
				.getSelectedItem();
		...
		for (String s : map.keySet()) {
			...
			for (String item : items) {
				...
				String statement = vennInfo.getStatements().get(
						Math.abs(statementNo));
				if (statement != null)
					info.append("<font "
							+ (statementNo < 0 ? "color=\"red\">" : "/>")
							+ "<tr><td valign=\"top\">" + "<b>"
							+ Math.abs(statementNo)
							+ "</b></td><td valign=\"top\"> " + statement
							+ "</font></td></tr></font>");
			}
		...
		confidenceLabel.setText("Confidence "
				+ new DecimalFormat("0.0").format(confidenceValue) + "%");
		thresholdLabel.setText("Threshold "
				+ new DecimalFormat("0.00").format(thresholdSE) + " SE = "
				+ new DecimalFormat("0.00").format(thresholdValue));
		repaint();
	}

	private float getThresholdSE() {
		return threshold.getValue() / (float) threshold.getMaximum()
				* maxStandardErrors;
	}

	private void setThresholdSE(float thresholdSE) {
		threshold.setValue(Math.round(thresholdSE / maxStandardErrors
				* threshold.getMaximum()));
	}
}
